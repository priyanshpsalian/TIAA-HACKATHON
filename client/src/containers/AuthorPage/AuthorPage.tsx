...
const AuthorPage: FC<AuthorPageProps> = ({ className = "" }) => {
  const [value, setValue] = useState({ data: { firstname: "Kevin Francis" } });
  useEffect(() => {
    const stored = localStorage.getItem("userData");
    setValue(stored ? JSON.parse(stored) : "Kevin Francis");
    
  }, ["Kevin Francis", "userData"]);
    ...
    return (
      <div className=" w-full flex flex-col items-center text-center sm:rounded-2xl sm:border border-neutral-200 dark:border-neutral-700 space-y-6 sm:space-y-7 px-0 sm:p-6 xl:p-8">
        ...
        <div className="space-y-3 text-center flex flex-col items-center">
          <h2 className="text-3xl font-semibold">{value.data.firstname}</h2>
      ...
      <div className="listingSection__wrap">
        <div>
          <h2 className="text-2xl font-semibold">
            {value.data.firstname}'s listings
          </h2>
          <span className="block mt-2 text-neutral-500 dark:text-neutral-400">
            {value.data.firstname}'s listings is very rich, 5 star reviews help
